import { CacheManagerImplementation, HuulkHttpMethods, ONE_DAY_IN_MS } from "../HuulkTypes";
import { HuulkSessionManager } from "../SessionManager/HuulkSessionManager";
import process from "process";
import { HuulkConfigurerFacade } from "./HuulkBuilder";
const HULK_SESSION_SIZE = 7;
let huulkRedisOptions = new HuulkConfigurerFacade()
    .startConfiguration()
...
test("Testing logging out deletes session from db", async () => {
    ...
    expect(testSession).toBeTruthy();
    //@ts-ignore
    let deleteSuccess = await redisManager.deleteSession(testSession.id.valueOf(), "loginTest");
...
test("Test logging twice creates two sessions with correct keys metadata", async () => {
    ...
    testSession = await redisManager.createSession("loginTest", huulkRedisOptions.cookieOptions, testAuthorities);
    keys.push(testSession.id.toString());
    let metaData = await redisManager.getSertMetadata("loginTest");
    //@ts-ignore
